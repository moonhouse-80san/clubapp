...
async function toggleAttendance(memberIndex) {
    ...
    // 체크 전 상태 기억 (UI 알림 판단용)
    const wasAlreadyChecked = member.attendanceDates.includes(selectedDate);

    // ── DB 저장 (AttendanceAPI.toggle) ──
    try {
        const result = await AttendanceAPI.toggle(member.id, selectedDate);

        // 서버가 최신 상태를 반환하면 반영, 없으면 직접 계산
        if (result.attendance_dates !== undefined) {
            member.attendanceDates = result.attendance_dates;
        } else {
            // 폴백: 직접 토글
            if (wasAlreadyChecked) {
                const idx = member.attendanceDates.indexOf(selectedDate);
                if (idx !== -1) member.attendanceDates.splice(idx, 1);
    ...
    if (wasAlreadyChecked) {
        ...
        } else if (targetCount > 0 && member.currentCount === 0) {
            // 목표 달성 후 서버에서 자동 초기화된 경우
            renderMembers();
